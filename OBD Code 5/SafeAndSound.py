...
# Getting Refernce Distance
hf.getRefDistance()

# create a connection with ECU
ecu.obdThread()

# Give time for ECU connection before starting GUI
while not settings.ecuReady :
    time.sleep(.02)
...
# Program Loop
while(True):
    # log car data into system database every 1 second
    hf.logSystem()
    ...
    if(settings.speed > 0 and not settings.movingFlag):
        ...
        x = datetime.today()
        settings.tripStartTime = str(x.day)+"-"+str(x.month)+"-"+str(x.year)+"/"+str(x.hour)+":"+str(x.minute)+":"+str(x.second)
        hf.startNewTrip(settings.tripStartTime)
	...
	index2 = hf.getTach2()
	    ...
	    hf.display_image(settings.tach_images[index1],320,40)
            hf.display_image(settings.tach_images[index2],85,l40)
	    pygame.draw.rect(settings.displayObj,white,(10,10,150,100),2)
            pygame.draw.rect(settings.displayObj,white,(640,10,150,100),2)
            pygame.draw.rect(settings.displayObj,white,(640,370,150,100),2)
            pygame.draw.rect(settings.displayObj,white,(10,370,150,100),2)
